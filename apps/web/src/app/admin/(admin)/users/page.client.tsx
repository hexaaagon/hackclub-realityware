...
export default function UsersPageClient({ data }: { data: Extract<InferResponseType<typeof client.admin.users.$get>, { success: true }> }) {
  return (
              ...
              <TableCell className="hidden md:table-cell">
                <div className="flex items-center gap-2">
                  {(item.permissions || ["member"]).map((permission) => (
                    <Badge
                      key={permission}
                      variant={permission === "member" ? "outline" : "default"}
                    >
                      {permission}
                    </Badge>
                  ))}
